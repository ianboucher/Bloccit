 ...
 RSpec.describe Api::V1::UsersController, type: :controller do
   ...
   context "unauthenticated users" do
     ...
     it "PUT update returns http unauthenticated" do
       new_user = build(:user)
       put :update, id: my_user.id, user: { name: new_user.name,
     ...
     it "POST create returns http unauthenticated" do
       new_user = build(:user)
       post :create, user: { name: new_user.name, email: new_user.email,
   ...
   context "authenticated and unauthorized users" do
     ...
     it "POST create returns http forbidden" do
       new_user = build(:user)
       post :create, user: { name: new_user.name,
         email: new_user.email, password: new_user.password }
       expect(response).to have_http_status(403)
     end
   end
   ...
   context "authenticated and authorized users" do
     ...
     describe "GET index" do
       ...
       # Note content type is a header and is added by the Bloccit API
     ...
     describe "PUT update" do
       context "with valid attributes" do
         before do
          # build a new user with FactoryGirl
           @new_user = build(:user)
          # before each spec, we send a request to update my_user with the attributes
          # of new_user
           put :update, id: my_user.id, user: { name: @new_user.name,
             email: @new_user.email, password: @new_user.password, role: "admin" }
         end
       ...
       context "with invalid attributes" do
         before do
           put :update, id: my_user.id, user: { name: "", email: "bademail@",
             password: "short" }
         end
     ...
     describe "POST create" do
       context "with valid attributes" do
         before do
         # build new_user with FactoryGirl then request to create a user with the
         # attributes of new_user
           @new_user = build(:user)
           post :create, user: { name: @new_user.name, email: @new_user.email,
             password: @new_user.password, role: "admin" }
         end
